...
bool CartesianPoseExampleController::init(hardware_interface::RobotHW* robot_hardware,
  ...
  try {
    ...
    for (size_t i = 0; i < q_start.size(); i++) {
      if (std::abs(state_handle.getRobotState().q_d[i] - q_start[i]) > 10 ) { //1-0.1) {
        ROS_ERROR_STREAM(
...
void CartesianPoseExampleController::optPoseCallback(const std_msgs::Float64MultiArray& msg) {
