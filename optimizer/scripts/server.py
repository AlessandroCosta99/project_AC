...
class PushingController:
	...
	def callback(self, robot_poes_msg, haptic_finger_msg):
		...
		if self.stop == 0:
			...
			if self.time_step > 6:
				service = rospy.Service("predictor", pred, self.create_input_for_model)
				print("t = {} --> Ready to receive actions from client.".format(self.time_step))
		self.time_step +=1
	...
	def return_stem_position(self):
		print("we are at return stem position")
		stem_pose = self.get_stem_position()
		stem_pose_msg = Float64MultiArray(data=stem_pose)
		print("STEM POSE MSG: ",stem_pose_msg)
		return predResponse(stem_pose_msg)
